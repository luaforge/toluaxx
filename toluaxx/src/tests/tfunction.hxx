...
public:
  ...
  void set (float v[2]=0){ m_x = v[0], m_y=v[1]; }
  void setpointer (Point* p){ *this = *p; }
  void setref (Point& p){ *this = p; }
  void setvalue (Point p){ *this = p; }
  void setconst (const Point* p){ *this = *p; }
  void setname (const char* s){ strncpy(m_s,s,63); }
  
  void get (float* x, float* y) const{ *x = m_x, *y = m_y; }
  void get (float v[2]) const{ v[0] = m_x, v[1] = m_y; }
  Point* getpointer (){ return this; }
  Point& getref (){ return *this; }
  Point getvalue (){ return *this; }
  const Point* getconst () const{ return this; }
  const char* getname () const{ return m_s; }

  Point operator+ (const Point& p) const{ return Point(m_x+p.m_x,m_y+p.m_y); }
  Point operator- (const Point& p) const{ return Point(m_x-p.m_x,m_y-p.m_y); }
  Point operator- () const{ return Point(-m_x,-m_y); }
  Point operator* (const Point& p) const{ return Point(m_x*p.m_x,m_y*p.m_y); }
  Point operator/ (float n) const{ return Point(m_x/n,m_y/n); }
  Point operator^ (const Point& p) const{ return Point(m_x*p.m_y,m_y*p.m_x); }
  Point operator% (const Point& p) const{ return Point(m_x+p.m_y,m_y+p.m_x); }
  float operator| (const Point& p) const{ return (m_x-p.m_x)*(m_y-p.m_y); } // concat
  float operator~ () const{ return m_x+m_y; } // len operator
...
inline Point sub (const Point& p1, const Point& p2){ return p1-p2; }
inline Point mult(const Point& p1, const Point& p2){ return p1*p2; }
inline Point div (const Point& p1, float n){ return p1/n; }

inline void getpoint (const Point* p, float* x, float* y){ p->get(x,y); }
inline void setpoint (Point* p, float x=0, float y=0){ p->set(x,y); }
...
inline	void copyvector (int n, const Point v[], Point u[]=0){
  for (int i=0; i<n; ++i) u[i] = v[i];
}

inline Order invert (Order o){
  if (o==FIRST) return SECOND;
  else return FIRST;
}
...
// test call lua function as c++ method
#include"toluaxx.h"
class Object{  //tolua_export
 public:
  ...
  void step(double period){
    tolua_callmethod(Object,handler,
